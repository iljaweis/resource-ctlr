...
func Ready(c client.Client, namespace string, req *resourcesv1alpha1.Requires) (bool, error) {
	...
	for _, r := range *req {
		...
		if r.File != nil {
			...
			err := c.Get(context.TODO(), types.NamespacedName{Namespace: namespace, Name: r.Command.Name}, x)
			if err != nil && !errors.IsNotFound(err) {
				return false, pkgerrors.Wrapf(err, "could not fetch file %s/%s", namespace, r.Command.Name)
			}
			if err == nil && x.Status.Done {
				thisready = true
			}
		...
		if r.FileContent != nil {
			...
			err := c.Get(context.TODO(), types.NamespacedName{Namespace: namespace, Name: r.Command.Name}, x)
			if err != nil && !errors.IsNotFound(err) {
				return false, pkgerrors.Wrapf(err, "could not fetch filecontent %s/%s", namespace, r.Command.Name)
			}
			if err == nil && x.Status.Done {
				thisready = true
			}
